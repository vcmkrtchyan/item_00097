        ...
        <div className="space-y-3">
          ...
          {trip.destinations.length > 0 && (
            <div className="flex items-center text-sm text-muted-foreground">
              ...
              <span className="line-clamp-1">{trip.destinations.map((d) => d.name).join(" → ")}</span>
            </div>
          )}

          {trip.description && <p className="text-sm text-muted-foreground line-clamp-2 mt-2">{trip.description}</p>}
        </div>
      </CardContent>
